...
//
// Мьютекс держится через синхронное обращение к базе (на промах кэша или по
// истечении TTL), потому что единственный потребитель — обновления в engine
// (движок работает в одной горутине, go func нет). Админка обращается
// напрямую к *Store. Если когда-нибудь появится второй конкурентный читатель,
// этот мьютекс будет первое, что нужно переделать.
type Cache struct {
	store *Store
	ttl   time.Duration
	now   func() time.Time
